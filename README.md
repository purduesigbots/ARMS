...
All configuration of the library will happen in the file `ARMS/config.h`. These are the settings we can use to configure how the chassis motors perform. Start by configuring the motor ports according to what ports your chassis motors are plugged into.
```
...
There are three default operator control modes supported, tank and arcade. Put one of the following into your opcontrol loop:
```
chassis::arcade(master.get_analog(ANALOG_LEFT_Y) * (double)100 / 127,
		            master.get_analog(ANALOG_RIGHT_X) * (double)100 / 127);

chassis::tank(master.get_analog(ANALOG_LEFT_Y) * (double)100 / 127,
		            master.get_analog(ANALOG_RIGHT_Y) * (double)100 / 127);
...
### Tuning the drive
The above chassis movements probably won't work right out of the box. To fix that you might need to make a few adjustments.
Near the top of config.h there are two important constants we need to adjust.
```
#define DISTANCE_CONSTANT 45 // ticks per inch
#define DEGREE_CONSTANT 2.3 // ticks per degree
```

`DISTANCE_CONSTANT` correlates to the `chassis::move()` function and `DEGREE_CONSTANT` correlates to the `chassis::turn()` function.

To tune the constants, tell the robot to drive 1 field tile of distance in autonomous.
```
chassis::drive(24); // measured in inches
```  
Adjust the distance constant until the robot travels to the correct target. When you tell the robot to `drive(2.5);`, the distance constant determines if the robot drives 2.5 meters, 2.5 feet, 2.5 inches, or some other distance.
...
For instance, we could drive 2 feet, then start running our intake, and keep driving another 1 foot. If we did this with the regular movement function it would look like this.
```
chassis::move(24);
intake.move(127);
chassis::move(12);
intake.move(0);
```
The problem with this method is that the robot will come to a complete stop before it starts running the intake. If we want the robot to keep driving without stopping we need to use the function `chassis::fast`. This will keep the robot from stopping. Here is an example:
```
chassis::fast(24);
intake.move(127);
chassis::move(12);
...
The chassis::fast function can also be used to slow down the robot when approaching an object too fast. For example, if we want to pick up a game object that is 3 feet away from the robot, we might drive the first 2 feet at top speed, then slow down as we approach the game object.
```
chassis::fast(24);
intake.move(127);
chassis::move(12, 50);
...
In autonomous, if you want to move the drive for duration of time rather than a distance, you can use the `chassis::voltage` function.
`chassis::voltage(1000, 50);`
...
This type of movement can also be produced with a velocity based movement instead of a voltage based one. `chassis::velocity(1000, 50);` This will used the velocity PID that is built-in to the the v5 motors. 

If you want to send different power to each side of the drive, call the function with two separate speeds.
`chassis::voltage(1000, 50, 80);`
...
### S-Curves
These are the most efficient way to move a robot in two dimensions. To do an S-shaped movement would be slow and take several `chassis::move()` and `chassis::turn()`
...
```
chassis::sLeft(800, 200, 1200);
chassis::sRight(800, 200, 1200);
```
